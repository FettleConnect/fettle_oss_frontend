...
export const PatientView: React.FC<PatientViewProps> = ({ user, onLogout }) => {
  ...
  const sanitizeImages = (images?: string[]) =>
    (images ?? []).filter((img) => typeof img === 'string' && img.trim().length > 10);
  ...
  const isPrivacyFlagMessage = (content: string) => {
    ...
    return (
      lower.includes('these images may contain identifiable personal information') ||
      lower.includes('intake is paused')
    );
  };
  ...
  const isFaceDetectedMessage = (content: string) => {
    ...
    return (
      ...
      lower.includes('face or identifiable')
    );
  };
  ...
  const appendImagesToFormData = async (
    ...
    for (let i = 0; i < safeImages.length; i++) {
      try {
        const blob = await dataURLtoBlob(safeImages[i]);
  ...
  const handlePrivacyRemoveImages = () => {
    ...
    toast({
      ...
      description: 'Please re-upload images without identifiable information.',
    });
  };
  ...
  const handlePaymentSuccess = async () => {
    ...
    setTimeout(() => {
      setMessages((prev) => {
        if (prev.some((m) => m.content === INTAKE_QUESTIONS[0])) {
          return prev;
        }
    ...
    if (mode === 'post_payment_intake') {
      if (intakeStep === 0) {
        ...
        try {
          ...
          if (sanitizedImages.length) {
            await appendImagesToFormData(formData, sanitizedImages);
          }
          ...
          if (resultText && isFaceDetectedMessage(resultText)) {
            clearUnsafeImages();
            ...
            setMessages((prev) => [
              ...prev,
              {
                ...
                role: 'AI',
                content:
                  '⚠️ Face or identifiable content detected. Please upload only clinical skin images without faces, names, or personal details.',
              },
            ]);
          ...
          if (resultText && isPrivacyFlagMessage(resultText)) {
            clearUnsafeImages();
      ...
      if (intakeStep === 1) {
        ...
        try {
          ...
          if (resultText && isReportPiiMessage(resultText)) {
            setIntakeData((prev) => ({
              ...prev,
              reportImages: [],
            }));
      ...
      if (isLastStep) {
        ...
        try {
          ...
          const finalIntakeData = {
            ...intakeData,
            ...(currentStepKey !== 'images' && currentStepKey !== 'reportImages'
              ? { [currentStepKey]: content }
              : {}),
          };

          const allImages = [
            ...sanitizeImages(finalIntakeData.images),
            ...sanitizeImages(finalIntakeData.reportImages),
          ];

          formData.append(
            'question',
            `INTAKE COMPLETE. Summary:
Duration: ${finalIntakeData.duration}
Symptoms: ${finalIntakeData.symptoms}
Location: ${finalIntakeData.location}
Meds: ${finalIntakeData.meds}
History: ${finalIntakeData.history}
Skin images: ${sanitizeImages(finalIntakeData.images).length} attached.
Report images: ${sanitizeImages(finalIntakeData.reportImages).length} attached.
DONE`
          );
    ...
    try {
      ...
      setMessages((prev) => {
        if (
          prev.some(
            (m) => m.content === aiContent && (m.role === 'ai' || m.role === 'AI')
          )
        ) {
          return prev;
        }
