...
// ── Face / PII detection (doctor side) ───────────────────────────────────────
// FIX (Bug 2): On API failure or JSON parse error we NO LONGER hard-block.
// Transient network errors should not prevent a doctor from attaching a clinical
// photo. The backend performs an independent check before storing any image.
...
Reply with JSON only. No explanation outside the JSON.`,
    ...
    // FIX (Bug 2): API failure → allow image through (backend is the authoritative gate).
    if (!response.ok) {
      console.warn('Doctor detectFaceOrPII: API error', response.status, '— allowing image (backend will re-check)');
    ...
    try {
      ...
      console.warn('Doctor detectFaceOrPII: JSON parse error — allowing image:', parseError);
      return { blocked: false, reason: '' };
    }
    ...
    // FIX (Bug 2): Network/exception → allow through.
    console.warn('Doctor detectFaceOrPII: exception — allowing image:', err);
    return { blocked: false, reason: '' };
  }
}
...
function isPlaceholder(text: string): boolean {
  ...
  const placeholders = [
    'type here', 'enter response', 'write response', 'draft response',
...
function normalizeAIContentToStructuredFormat(rawText: string): string {
  ...
  if (!finalText) return DEFAULT_ASSESSMENT_TEMPLATE;
...
function extractSections(text: string): Record<string, string> {
  ...
  return extractLegacyNumberedSections(text);
}
...
function mergeStructuredContent(existingText: string, aiText: string): string {
  ...
  const mergedBlocks = SECTION_TITLES.map(title => {
    ...
    if (!existing || isPlaceholder(existing)) finalBody = ai;
    if (title === 'References' && !finalBody) finalBody = ai;
...
export const DoctorChatView: React.FC<DoctorChatViewProps> = ({
  ...
  const handleApplyDraft = () => {
    ...
    toast({ title: 'Applied to Editor', description: 'Structured AI content was merged into the Assessment & Response box.' });
  };
  ...
  const handleRegenerateDraft = async () => {
    ...
    try {
      ...
      toast({ title: 'Refreshing AI Analysis', description: 'A new structured analysis is being generated. Click Sync in a few seconds.' });
      setTimeout(() => { onRefresh(); }, 12000);
    } catch (error) {
      console.error('Regenerate draft error:', error);
      toast({ title: 'Error', description: 'Failed to refresh AI analysis.', variant: 'destructive' });
    } finally {
      setIsRegenerating(false);
    }
  };

  // FIX (Bug 2 + Bug 3): Image selection with fail-safe fix and deduplication.
  // detectFaceOrPII no longer hard-blocks on API/network errors.
  // Deduplication prevents adding the same image twice across multiple picks.
  const handleImageSelect = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
          ...
          file =>
            new Promise<void>(resolve => {
              ...
              reader.onload = async ev => {
                ...
                if (blocked) {
                  rejected.push(`"${file.name}" — ${reason || 'contains a face or personal information'}`);
                } else {
      ...
      // FIX (Bug 3): Deduplicate by dataUrl content so picking the same file
      // twice (or across multiple open-file dialogs) never adds a duplicate.
  ...
  // FIX (Bug 3): Hard removal — fresh array reference so React always re-renders
  // and the deleted image cannot be included in a subsequent send.
  ...
  const handleSendToPatient = async () => {
    // FIX (Bug 3): Guard against double-send (isSending flag).
    if (isSending) return;

    if (!patientMessage.trim() && images.length === 0) {
      toast({ title: 'Error', description: 'Please enter a message or attach an image.', variant: 'destructive' });
      return;
    }
    ...
    try {
      ...
      // FIX (Bug 3): Snapshot images at send time.
      // Any image the doctor removed before clicking Send is excluded.
      ...
      // FIX (Bug 3): Clear images ONLY after a successful send.
  ...
  const handleApplyAIContent = useCallback(
    (content: string) => {
      ...
      setPatientMessage(merged);
      toast({ title: 'Assessment Updated', description: 'Structured AI content was applied to the editor.' });
                ...
                <div className="flex items-center justify-between mb-1">
                  ...
                  <div className="flex gap-1.5 md:gap-2 flex-wrap justify-end">
                    ...
                    <Button
                      ...
                      className="h-7 md:h-8 px-2 md:px-3 gap-1 md:gap-2 text-[10px] md:text-xs"
                    >
                      {assessmentExpanded ? <Minimize2 className="h-3.5 w-3.5" /> : <Maximize2 className="h-3.5 w-3.5" />}
                      <span className="hidden sm:inline">
